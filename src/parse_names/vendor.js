...
goog.provide("parse_names.vendor");

parse_names.vendor.removeIgnoredChars = function (word) {
    //ignore periods
    return word.replace(".","");
};

// detect compound last names like "Von Fange"
parse_names.vendor.is_compound_lastName = function (word) {
...
parse_names.vendor.is_camel_case = function (word) {
    var ucReg = /[A-Z]+/;
    var lcReg = /[a-z]+/;
...
parse_names.vendor.safe_ucfirst = function (seperator, word) {
    return word.split(seperator).map(function(thisWord){
	if(parse_names.vendor.is_camel_case(thisWord)) {
	    return thisWord;
	} else {
...
parse_names.vendor.fix_case = function (word) {
    // uppercase words split by dashes, like "Kimura-Fay"
    word = parse_names.vendor.safe_ucfirst("-",word);
...
parse_names.vendor.is_salutation = function (word) {
    word = parse_names.vendor.removeIgnoredChars(word).toLowerCase();
...
parse_names.vendor.is_suffix = function (word) {
    word = parse_names.vendor.removeIgnoredChars(word).toLowerCase();
...
parse_names.vendor.parse = function (fullastName) {
    ...
    var salutation = parse_names.vendor.is_salutation(nameParts[0]);
    var suffix = parse_names.vendor.is_suffix(nameParts[numWords - 1]);
    ...
    if (parse_names.vendor.is_initial(word)) {
	// if so, do a look-ahead to see if they go by their middle name
	// for ex: "R. Jason Smith" => "Jason Smith" & "R." is stored as an initial
	// but "R. J. Smith" => "R. Smith" and "J." is stored as an initial
	if (parse_names.vendor.is_initial(nameParts[start + 1])) {
	    firstName += " " + word.toUpperCase();
	} else {
	    initials += " " + word.toUpperCase();
	}
    } else {
	firstName += " " + parse_names.vendor.fix_case(word);
    }
	...
	if (parse_names.vendor.is_compound_lastName(word)) {
	    break;
	}
	if (parse_names.vendor.is_initial(word)) {
	    initials += " " + word.toUpperCase();
	...
	for (j=i; j<end; j++) {
	    lastName += " " + parse_names.vendor.fix_case(nameParts[j]);
	}
    }
